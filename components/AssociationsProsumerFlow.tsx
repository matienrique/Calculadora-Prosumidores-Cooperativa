    ...
    <div className="space-y-8 animate-fadeIn max-w-4xl mx-auto pb-20">
      ...
      <div className={sectionClass}>
        ...
        <div className="pt-6 border-t grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div><label className={labelClass}>TOTAL A PAGAR ($) *</label><input type="number" step="any" className={`${inputClass} border-blue-500 bg-blue-50 font-black text-xl shadow-inner`} value={formData.totalToPay} onChange={(e) => handleInputChange('totalToPay', e.target.value)} /></div>
        </div>
      </div>

      <div className={sectionClass}>
        <h3 className="text-lg font-black text-blue-600 border-b pb-2 mb-4 uppercase italic tracking-tighter">IMPUESTOS/GRAVÁMENES</h3>
      ...
      <div className="flex justify-center pt-8">
        <button onClick={() => setShowResults(true)} className="bg-red-600 hover:bg-red-700 text-white font-black py-6 px-20 rounded-full shadow-2xl hover:shadow-red-200 transition-all duration-300 transform hover:-translate-y-2 uppercase tracking-widest text-lg">
          Calcular
        </button>
      </div>
    </div>
