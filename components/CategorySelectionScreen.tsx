...
import { UserCategory, ProsumerStatus } from '../types';
...
const CategorySelectionScreen: React.FC<Props> = ({ onSelect, onBack, prosumerStatus }) => {
  const categories = [
    {
  ...
  if (prosumerStatus !== null) {
    categories.push({
      id: UserCategory.ASSOCIATIONS,
      title: "Asociaciones",
      desc: "Entidades sin fines de lucro y asociaciones civiles.",
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  const getColorClasses = (color: string) => {
    switch (color) {
      ...
      case 'pink': return "bg-pink-50 text-pink-600 group-hover:bg-pink-500";
  ...
  const getBorderClasses = (color: string) => {
    switch (color) {
      ...
      case 'pink': return "hover:border-pink-500";
