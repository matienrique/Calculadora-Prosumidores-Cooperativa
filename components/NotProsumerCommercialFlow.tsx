...
const NotProsumerCommercialFlow: React.FC<Props> = ({ onBack }) => {
  ...
  const results = useMemo(() => {
    ...
    const inversionInicial = maxPower * tipoCambio * inversionNoResidencial;
  ...
  }, [formData, fiscalData, tipoCambio, inversionNoResidencial]);
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const canvas = await html2canvas(element, {
        ...
        onclone: (clonedDoc) => {
          const el = clonedDoc.getElementById('results-content');
      ...
      pdf.save('resultados-comercial.pdf');
    } catch (error) {
  ...
  const inputClass = "w-full p-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-emerald-600 outline-none transition-all bg-white shadow-sm";
      ...
      <div id="results-content" className="space-y-8 animate-fadeIn max-w-4xl mx-auto pb-20">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-2xl font-black text-slate-800 uppercase italic">Resultados Comerciales</h2>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <div className="bg-gradient-to-r from-[#FF5F6D] to-[#B83AF3] p-6 rounded-3xl shadow-xl text-center text-white">
            <p className="text-emerald-100 text-[10px] font-black uppercase mb-1 tracking-widest">Factura estimada (Prosumidor)</p>
        ...
        <div className="text-center bg-emerald-50 p-4 rounded-2xl border border-emerald-100 mb-6">
          <p className="text-xs font-black text-emerald-800 uppercase tracking-widest">Potencia maxima de instalacion</p>
          <p className="text-2xl font-black text-emerald-600">{results.maxPower.toFixed(2)} kW</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-gradient-to-r from-[#FF5F6D] to-[#B83AF3] p-6 rounded-3xl shadow-xl text-center text-white flex flex-col justify-center">
            <p className="text-emerald-100 text-xs font-black uppercase mb-2 tracking-widest">Período de recuperación</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            ...
            <div className="flex flex-col items-center">
              <svg viewBox="0 0 36 36" className="w-44 h-44 drop-shadow-md">
                ...
                <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="#10b981" strokeWidth="3" strokeDasharray={`${Math.round(pAuto)}, 100`} />
                <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="#3b82f6" strokeWidth="3" strokeDasharray={`${Math.round(pImp)}, 100`} strokeDashoffset={`-${Math.round(pAuto)}`} />
                <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="#f59e0b" strokeWidth="3" strokeDasharray={`${Math.round(pRec)}, 100`} strokeDashoffset={`-${Math.round(pAuto + pImp)}`} />
              </svg>
              <div className="mt-4 flex gap-4 text-[9px] font-black uppercase text-slate-400">
                <span className="flex items-center"><div className="w-2 h-2 bg-emerald-600 mr-1"></div> {Math.round(pAuto)}% Auto.</span>
                <span className="flex items-center"><div className="w-2 h-2 bg-blue-500 mr-1"></div> {Math.round(pImp)}% Imp.</span>
                <span className="flex items-center"><div className="w-2 h-2 bg-amber-500 mr-1"></div> {Math.round(pRec)}% GSF</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white p-6 rounded-3xl border border-slate-200">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden"><div className="bg-emerald-600 h-full" style={{width: `${safeDiv(results.autoconsumoKwh, results.EG) * 100}%`}}></div></div>
              </div>
              <div>
          ...
          <div className="bg-slate-900 p-6 rounded-3xl text-white shadow-xl flex flex-col justify-center">
            <h3 className="text-[10px] font-black text-emerald-400 uppercase tracking-[0.2em] mb-4 italic">Impacto Ambiental Positivo</h3>
            <div className="grid grid-cols-3 gap-4 text-center">
              ...
              <div><p className="text-slate-500 text-[8px] font-black uppercase mb-1">CO2 Evitado</p><p className="text-xl font-bold text-emerald-400">{(results.EG * 0.2306).toFixed(2)} <span className="text-[10px] font-normal text-slate-400">kg</span></p></div>
              <div><p className="text-slate-500 text-[8px] font-black uppercase mb-1">Árboles eq.</p><p className="text-xl font-bold text-blue-400">{Math.ceil((results.EG * 0.2306) / (10/12))}</p></div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-12">
          <FeedbackForm userType="No Prosumidor Comercial" />
        </div>
    ...
    <div className="space-y-8 animate-fadeIn max-w-4xl mx-auto pb-20">
      ...
      <div className="bg-gradient-to-r from-[#FF5F6D] to-[#B83AF3] p-6 rounded-3xl text-white flex items-center justify-between shadow-xl">
        <div className="flex items-center space-x-3">
          ...
          <span className="text-xs font-black uppercase tracking-widest italic">Simulación Comercial</span>
        </div>
        <div className="text-right">
          <p className="text-[10px] font-black uppercase text-emerald-200 tracking-tighter">Autoconsumo estimado</p>
          <p className="text-2xl font-black">75%</p>
        </div>
      </div>

      <div className={sectionClass}>
        <h3 className="text-lg font-black text-emerald-600 border-b pb-2 mb-4 italic uppercase tracking-tighter">HISTORIA DE CONSUMO DE ENERGÍA</h3>
        ...
        <div className="space-y-8">
          ...
          {(formData.knowsPower !== null) && (
            <div className="pt-6 border-t border-slate-100 animate-fadeIn">
              ...
              <p className="mt-2 text-[10px] font-bold text-slate-400 uppercase italic leading-relaxed border-l-4 border-emerald-200 pl-4 py-1">
      ...
      <div className={sectionClass}>
        <h3 className="text-lg font-black text-emerald-600 border-b pb-2 mb-4 italic uppercase tracking-tighter">DETALLE DE SU FACTURA</h3>
        ...
        <div className="space-y-6">
          <div>
            ...
            {formData.fiscalStatus && <div className="mt-3 bg-emerald-50 p-4 rounded-xl border border-emerald-100 text-[10px] font-black text-emerald-700 uppercase tracking-widest animate-fadeIn">{fiscalData.label}</div>}
          </div>
          ...
          <div className="border border-slate-100 rounded-3xl overflow-hidden shadow-sm">
            ...
            {showFixedCharges && (
              <div className="p-8 space-y-4 bg-white border-t animate-fadeIn">
                ...
                <button onClick={() => addItem('fixedCharges')} className="text-emerald-600 font-black text-xs uppercase tracking-widest">+ Agregar Cargo</button>
              </div>
            )}
          </div>
...
export default NotProsumerCommercialFlow;
