  ...
  if (showResults) {
    ...
    return (
      <div id="results-content" className="space-y-8 animate-fadeIn max-w-4xl mx-auto">
        ...
        <div className="mt-12">
          <FeedbackForm userType="Prosumidor Residencial" />
        </div>
    ...
    <div className="space-y-8 animate-fadeIn max-w-4xl mx-auto pb-20">
      ...
      <div className={sectionClass}>
        ...
        <div className="pt-6 border-t grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="md:col-span-2"><label className={`${labelClass} text-blue-600`}>TOTAL A PAGAR ($) *</label><input type="number" step="any" className={`${inputClass} border-blue-600 bg-blue-50 font-black text-xl shadow-inner`} value={formData.totalToPay} onChange={(e) => handleInputChange('totalToPay', e.target.value)} /></div>
        </div>
      </div>
